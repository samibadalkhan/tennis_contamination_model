"""Protocol-corrected WTA 2025 replication excluding team events."""
...
OUT = RESULTS / "wta_2025" / "corrected"
FIT_DIR = OUT / "fits"
CACHE_DIR = DATA / "wta_2025_corrected_cache"
PROTOCOL = OUT.parent / "PROTOCOL.md"
DEVIATION = OUT.parent / "PROTOCOL_DEVIATION.json"
LEVELS = {"G", "PM", "P", "I", "F"}
TEAM_EVENT_PATTERN = r"(?:United Cup|Fed Cup|Billie Jean King Cup|Hopman Cup)"
...
def team_event_mask(names: pd.Series) -> pd.Series:
    """Return the rows excluded by the frozen team-event rule."""
    return names.fillna("").str.contains(TEAM_EVENT_PATTERN, case=False, regex=True)
...
def load_wta(max_source_year: int) -> tuple[pd.DataFrame, dict]:
    ...
    for source_year in range(2011, max_source_year + 1):
        ...
        n_level = len(raw)
        team_event = team_event_mask(raw.tourney_name)
        n_team_event = int(team_event.sum())
        raw = raw.loc[~team_event].copy()
        ...
        audits.append({"source_year": source_year, "raw": n_raw, "eligible_level": n_level,
                       "excluded_team_event": n_team_event,
...
def run() -> dict:
    ...
    if not DEVIATION.exists():
        raise FileNotFoundError(DEVIATION)
    OUT.mkdir(parents=True, exist_ok=True)
    freeze = {"utc": utcnow(), "status": "frozen_protocol_correction_after_test_open",
              "periods": PERIODS, "levels": sorted(LEVELS), "mcmc_seed": MCMC_SEED,
              "burst_seed": BURST_SEED, "burst_rate": 0.5, "temperature": 1.5,
              "team_event_pattern": TEAM_EVENT_PATTERN,
              "protocol_sha256": hashlib.sha256(protocol.read_bytes()).hexdigest(),
              "deviation_sha256": hashlib.sha256(DEVIATION.read_bytes()).hexdigest(),
              "code_sha256": hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
              "test_source_sha256": manifest_entries()["wta/wta_matches_2025.csv"]["sha256"]}
    freeze_path = OUT / "FROZEN_CORRECTION.json"
    ...
    result = {"utc": utcnow(), "status": "constrained_protocol_correction", "n": len(primary),
...
def report(result: dict) -> None:
    lines = ["# Protocol-corrected WTA 2025 replication", "",
...
def plot(result: dict) -> None:
    ...
    fig.suptitle("Protocol-corrected WTA 2025 replication")
    fig.tight_layout()
